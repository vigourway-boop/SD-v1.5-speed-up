...
Stable Diffusion 的 CLIP、UNet、Scheduler 和 VAE 在 PC GPU 上运行。PC 每个扩散时间步将 CFG 精简后的 latent 特征量化为 int16，并通过以太网发送给 PYNQ-Z2。FPGA 计算点积、两个向量范数和余弦阈值结果，PYNQ ARM 返回具体余弦相似度与跳步指令，PC 据此执行或跳过 UNet。

```mermaid
flowchart LR
    A["PC GPU: SD 1.5"] --> B["int16 latent feature"]
...
pynq_cosine_client.py        PC 端 CSK2 二进制协议客户端
...
硬件测试会验证：首次无参考向量、相同向量余弦为 1、不同向量不通过阈值，以及 FPGA 结果与 PC int16 参考计算一致。
...
PYNQ-Z2 只负责特征统计和跳步控制。完整 CLIP、UNet 和 VAE 的参数量及带宽需求远超 Zynq-7020 的资源，因此仍由 PC GPU 执行。当前结果是一组验证实验，正式性能结论应继续使用多提示词、多随机种子统计。
